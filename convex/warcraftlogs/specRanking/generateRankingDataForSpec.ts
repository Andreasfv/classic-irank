...
import { getSpecRanking } from "./getSpecRankingForEncounter";
import { getActorsReport } from "../report/getReportActors";
import { getRankerActions, SpecKeys } from "../report/getRankerActions";
...
export const generateRankingDataForSpec = action({
    ...
    handler: async (ctx, args) => {
        const { className, spec, boss, difficulty, tenMan } = args as GenerateRankingDataForSpecInput;
        // @ts-ignore
        ...
        for (const ranker of specRankingData) {
            ...
            const rankerActorID = reportActors.report.masterData.actors.find(actor => actor.name === ranker.name)?.id
            const fightStartTime = reportActors.report.fights.find(fight => fight.id === ranker.report.fightID)?.startTime
            ...
            if (rankerActorID) {
                ...
                for (const action in rankerActions.data.reportData.report) {
                    const refactoredAbilities = rankerActions.data.reportData.report[action].data.map(ability => {
        ...
        await ctx.runMutation(internal.warcraftlogs.specRanking.createRankingEntry.createRankerEntries, {
