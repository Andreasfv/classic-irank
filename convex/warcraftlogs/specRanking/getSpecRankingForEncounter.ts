...
import { ClassName, Spec, WarcraftLogsdata } from "../types/consts";
...
interface SpecRankingOutput extends WarcraftLogsdata {
    data: {
        worldData: {
            encounter: {
                characterRankings: {
                    rankings: SpecRankingForEncounter[]
                }
        }
    }
    }
...
export async function getSpecRanking({className, spec, boss, difficulty, tenMan, accessToken}: SpecRankingInput): Promise<SpecRankingForEncounter[]> {
    ...
    if(!body?.data?.worldData?.encounter?.characterRankings) {
        ...
        console.log(body)
        throw new Error(`No spec rankings found for encounter id: ${boss}, spec: ${spec}, class: ${className}, difficulty: ${difficulty}`)
    }
